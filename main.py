...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        bookmarks = db.GqlQuery(
            "SELECT * " "FROM Bookmark WHERE user = :1",
            user.email() if user != None else None,
        )
    ...
    def post(self):
        ...
        try:
            ...
            image = urlfetch.Fetch(
                "http://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/No_icon_red.svg/582px-No_icon_red.svg.png"
            )
            img = images.Image(image.content)
        finally:
...
def delete_bookmark(user, name):
    bookmark = Bookmark.gql(
        "WHERE name = :1 AND user = :2", name, user.email() if user != None else None
    )
    db.delete(bookmark)
